    ...
    return (
        <div className="max-w-xs bg-slate-900 border border-slate-800 rounded-xl overflow-hidden shadow-2xl relative group" role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.currentTarget.click(); }>
            ...
            <a 
                href="#" 
                onClick={toggleLike} 
                className="absolute top-3 right-3 p-2 bg-slate-950/80 hover:bg-slate-950 rounded-full border border-slate-800 transition z-10" aria-label="Favorite">
            ...
            <div className="h-48 bg-slate-950 flex items-center justify-center overflow-hidden border-b border-slate-800">
                ...
                <img 
                    ...
                    className="object-cover h-full w-full group-hover:scale-105 transition duration-300" alt="A silver wristwatch with a white face, displayed on a wooden surface next to a book."/>
            </div>
